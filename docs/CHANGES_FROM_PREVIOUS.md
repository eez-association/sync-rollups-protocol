...
### `ActionType` enum
There is no longer an `ActionType` enum on `Action` or any other struct. All cross-chain work is expressed as flat call structs (`L2ToL1Call` on L1, `CrossChainCall` on L2); entry classification (immediate vs. deferred) is derived from `ExecutionEntry.proxyEntryHash == bytes32(0)` alone.
...
**New**: `revertSpan = N` on a call struct (`L2ToL1Call` on L1, `CrossChainCall` on L2) opens an isolated EVM context covering the next `N` calls. The processor self-calls `executeInContextAndRevert(N)`, which always reverts with `ContextResult(rollingHash, reentrantConsumed, callsProcessed, callNotFound)`. The EVM rolls back state inside the span; the counters escape via the revert payload and the outer flow restores them so the rolling hash and cursors reflect what happened inside (`callNotFound` forwards L1's deferred no-match flag; L2 always sends `false`).
...
- Reentrant call that **reverts** (caller catches with try/catch) → look up a `LookupCall` keyed by `(crossChainCallHash, l2ToL1CallNumber, lastL1ToL2CallConsumed)` (on L2: `callNumber` / `lastOutgoingCallConsumed`) with `failed = true`.
...
- `ScopeReverted` (error)
- `setRollupContract` (function) / `RollupContractChanged` (event) — the manager-handoff path was removed. A rollup's manager contract is set at registration time and is immutable thereafter (see the one-shot `rollupContractRegistered` natspec, `src/rollupContract/Rollup.sol:154-160`).
...
Note: `StateRootMismatch(uint256 rollupId)` and `StateDelta.currentState` were *briefly* removed in an earlier draft of this refactor but have been **re-added** as the soundness backstop — entries whose recorded `currentState` doesn't match `rollups[rid].stateRoot` at consumption time revert `StateRootMismatch` (`src/EEZ.sol:270, 1047`).
...
- **Reentry guard simplification**: the early `_inPostBatch` flag is gone. Reentry is detected via `_transientExecutions.length != 0` (cleared at the end of `postAndVerifyBatch`); a reentrant `postAndVerifyBatch` reverts `PostBatchReentry` (`src/EEZ.sol:157-161, 387`).
- **`setStateRoot` reentrancy guard (new)**: in addition to the same-block lockout, `setStateRoot` now reverts `SetStateRootNotAllowedDuringExecution` if `_insideExecution()` is true — the manager cannot rewrite state mid-execution via a reentrant proxy path (`src/EEZ.sol:1275-1283`).
...
| `CrossChainCall` (struct) | L1: `L2ToL1Call` (`IEEZ.sol`); L2 keeps `CrossChainCall` (`IEEZL2.sol`) |
| `NestedAction` (struct) | L1: `ExpectedL1ToL2Call`; L2: `ExpectedOutgoingCrossChainCall` |
| `ExecutionEntry.crossChainCallHash` (field) | `proxyEntryHash` (both sides) |
| `ExecutionEntry.calls` (field) | L1: `l2ToL1Calls`; L2: `incomingCalls` |
| `ExecutionEntry.nestedActions` (field) | L1: `expectedL1ToL2Calls`; L2: `expectedOutgoingCalls` |
...
| `CallResult`, `EntryExecuted`, `RevertSpanExecuted` | Declared per-side on `EEZ` and `EEZL2` (same shapes); `EEZBase` declares only the direction-neutral `CrossChainProxyCreated` / `CrossChainCallExecuted` |
| `NestedActionConsumed` | Renamed per-side: L1 `L1ToL2CallConsumed`, L2 `OutgoingCallConsumed` |
| `VerificationKeyUpdated`, `OwnershipTransferred` | Moved off the central registry — live on the per-rollup manager (`Rollup.sol` / OZ `Ownable`) |
| `StateUpdated`, `L2ExecutionPerformed` | Still on `EEZ` — emitted by `setStateRoot` and `_applyStateDeltas` respectively |
