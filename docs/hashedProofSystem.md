# Cross-Proof-System Interactions: Atomic Verification
...
## What Goes Into `crossProofSystemInteractions`

For a given proof system PS_k, the `crossProofSystemInteractions` is the hash of all **cross-proof-system** action hashes from the global ordered list that involve at least one rollup covered by PS_k.

Cross-proof-system means: the source rollup and destination rollup belong to different proof systems.

```
crossProofSystemInteractions[PS_A] = keccak256(
    actionHash(msg_0) ||   // PS_A -> PS_B
    actionHash(msg_2) ||   // PS_B -> PS_A
    actionHash(msg_3)      // PS_A -> PS_B
)

crossProofSystemInteractions[PS_B] = keccak256(
    actionHash(msg_0) ||   // PS_A -> PS_B
    actionHash(msg_2) ||   // PS_B -> PS_A
    actionHash(msg_3) ||   // PS_A -> PS_B
...
crossProofSystemInteractions[PS_C] = keccak256(
    actionHash(msg_4)      // PS_B -> PS_C
)
```
...
1. Compute its own view of the `crossProofSystemInteractions` from the execution it proves
2. Expose this hash as a public output
3. The on-chain verifier checks that the proof attests to the `crossProofSystemInteractions` value passed by the caller

Since `postBatch` requires ALL proofs to verify atomically:
- PS_A's proof commits to `crossProofSystemInteractions[PS_A]` which includes msg_0 (A->B)
- PS_B's proof commits to `crossProofSystemInteractions[PS_B]` which also includes msg_0 (A->B)
- If PS_A proves msg_0 was sent but PS_B's proof doesn't include msg_0 as received, PS_B's `crossProofSystemInteractions` won't match and its proof fails
- The batch reverts, preventing inconsistency

## On-Chain Integration

In `postBatch`, the `crossProofSystemInteractionss` array (one `bytes32` per proof system) is folded into each proof system's `publicInputsHash`:
...
publicInputsHash = keccak256(
    abi.encodePacked(
        ...
        crossProofSystemInteractionss[k]            // per-proof-system
    )
);
```

The contract does NOT independently verify the contents of `crossProofSystemInteractions` -- it trusts the ZK proof to attest correctness. The contract's role is to:
...
crossProofSystemInteractionss[PS_A] = keccak256(actionHash(msg_0))
crossProofSystemInteractionss[PS_B] = keccak256(actionHash(msg_0))
```
...
```
crossProofSystemInteractionss[PS_A] = bytes32(0)
```
...
Every cross-chain action in the `crossProofSystemInteractions` must carry the **initiator** — the original address (and rollup) that triggered the cross-chain call chain, not just the immediate source. This is important because:
...
- Mainnet (rollupId=0) as source/destination — mainnet has no proof system. Messages to/from mainnet are handled by L1 execution directly, so they probably don't belong in `crossProofSystemInteractions`. But this needs confirmation.
...
2. **Why global ordering?**
   Without a canonical order, two proof systems could include the same action hashes in different orders and produce different `crossProofSystemInteractions` values, breaking the shared-overlap property. Global ordering ensures consistency.
