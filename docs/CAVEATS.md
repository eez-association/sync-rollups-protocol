...
- **Sequential consumption is strict**: Entries are consumed in the exact order they were posted. `_consumeAndExecute` increments `executionIndex` (or `_transientExecutionIndex`) by one each call and reverts with `ExecutionNotFound` if the next entry's `crossChainCallHash` doesn't match the expected hash. There is no hash-based search and no swap-and-pop — a single out-of-order call from the prover invalidates the rest of the batch.
