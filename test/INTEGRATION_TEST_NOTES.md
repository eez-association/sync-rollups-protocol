...
|---|------|-----------|---------------|
| 1 | Alice -> A (-> B') -> B | L1 -> L2 | Simple: L1 contract calls L2 via proxy. Phase 1: L2 executes B via `executeIncomingCrossChainCall`. Phase 2: L1 resolves via `postVerifyAndExecuteOrSaveExecutionsFromBatch` + proxy call. |
...
`EEZ.sol` has no `executeIncomingCrossChainCall` (unlike `CrossChainManagerL2`). This is by design: **L1 always initiates L2 execution** — an L2 can receive execution from L1, but not the other way around. `executeL2TX` starts the L2 tx execution that interacts with L1.

How it works:
1. `postVerifyAndExecuteOrSaveExecutionsFromBatch` stores deferred entries with an L2TX action hash
...
This means the execution table needs entries for BOTH the outer call AND the inner reentrant call. For example, Scenario 3 Phase 1 needs 3 `postVerifyAndExecuteOrSaveExecutionsFromBatch` entries:
...
4. **Within each chain phase:** setup (postVerifyAndExecuteOrSaveExecutionsFromBatch/loadTable) comes before execution (call/executeL2TX/executeIncoming).
...
4. For each phase: first show table loading (postVerifyAndExecuteOrSaveExecutionsFromBatch on L1 / loadExecutionTable on L2), then show the execution that consumes those entries.
